...
test.describe('Domain Sourcing', () => {
	...
	test('product hunt source form shows max pages input and debug link', async ({ page }) => {
		await page.goto('/source');
		const productHuntCard = page.locator('article', {
			has: page.getByRole('heading', { name: 'Product Hunt' }),
		});
	...
	test('product hunt debug page loads with pre-filled maxPages and runs debug', async ({
		page,
	}) => {
